// Copia src/ a dist/, inyecta las variables de entorno y embebe el CSS y el JS
// en el HTML para que la página se pinte con una sola petición.
...
// Valores por defecto. Las variables de entorno solo sirven para cambiarlos.
...
// Snippet oficial de Meta. Quien ya rechazó las cookies entra con consent revoke
...
// Quita comentarios de línea completa e indentación.
...
async function build() {
  ...
  // Funciones de reemplazo, nunca cadenas: "$$" y "$&" corromperían el JS embebido.
  const out = html
    .replace("<!--CSS-->", () => `<style>\n${minifyCss(css)}\n</style>`)
    .replace("<!--JS-->", () => `<script>\n${js}\n</script>`)
  ...
  // El JS embebido debe ser idéntico al de src/main.js.
