// POST /api/lead — entrega el lead por email (Resend) y/o webhook (Google Sheets).
// Ambos canales son opcionales. Sin ninguno, el lead queda en los logs de Vercel.
// Variables de entorno: ver README.md

const MAX_BODY = 8 * 1024;
const WEBHOOK_TIMEOUT = 6000;
...
function validate(body) {
  ...
  // Solo se aceptan los parámetros conocidos, recortados.
...
// Campaña de procedencia para el email de aviso.
const campana = (lead) =>
...
async function sendWebhook(lead) {
  ...
  // Apps Script responde 200 aunque rechace el lead: hay que mirar el cuerpo.
  let datos = null;
...
export default async function handler(req, res) {
  ...
  // Estado de los canales, sin revelar claves.
  if (req.method === "GET") {
  ...
  // Rastro en los logs de Vercel como última red de seguridad.
